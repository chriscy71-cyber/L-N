...
# =====================================================================
# 1. ΑΡΧΙΚΟΠΟΙΗΣΗ ΥΠΟΔΟΜΗΣ & ΣΥΝΔΕΣΗ ΜΕ SUPABASE CLOUD (COLD START RECOVERY)
# =====================================================================
try:
...
def download_database_from_supabase():
    """Κατεβάζει το αρχείο database.db από το Supabase Storage κατά το ξύπνημα του server."""
...
# Εγγυημένη αυτόματη ανάκτηση της βάσης πριν από κάθε αίτημα (Λύνει το Render Sleep)
@app.before_request
...
def get_db_connection():
    ...
    return conn
# =====================================================================
# 2. ΔΟΜΗ ΒΑΣΗΣ ΔΕΔΟΜΕΝΩΝ & ΑΡΧΙΚΟΠΟΙΗΣΗ ΠΙΝΑΚΩΝ (FULL ENTERPRISE SCHEMA)
# =====================================================================
def init_db():
    """Δημιουργεί όλους τους απαραίτητους πίνακες και εισάγει τις βασικές ρυθμίσεις, κανάλια και προνόμια."""
    with get_db_connection() as conn:
        cursor = conn.cursor()
        
        # Πίνακας χρηστών με επίπεδα πρόσβασης, ρόλους και ειδικά δικαιώματα (GDPR & Hierarchy)
        ...
        # Μηνύματα και καταχωρήσεις καναλιών (Logbooks με mega links και συνημμένα)
        ...
        # Ιστορικό, βλάβες, ώρες λειτουργίας και επισκευές περιπολικών σκαφών
        ...
        # Ψηφιακή βιβλιοθήκη εγγράφων, βιβλίων και ανακοινώσεων
        ...
        # Γενικές ρυθμίσεις συστήματος, SMTP και Webhooks
...
# =====================================================================
# 3. ΔΙΑΧΕΙΡΙΣΗ ΑΡΧΕΙΩΝ, ΣΥΓΧΡΟΝΙΣΜΟΣ & FEDERATION WEBHOOKΣ
# =====================================================================
...
def clean_old_channel_entries():
    """Αυτόματη διαγραφή μηνυμάτων και αρχείων παλαιότερων των 60 ημερών (εκτός του Αρχείου)."""
...
def get_super_admin_phone_and_id():
    """Επιστρέφει το τηλέφωνο και το ID του Super Admin για έλεγχο ταυτότητας API."""
    with get_db_connection() as conn:
        cursor = conn.cursor()
...
def send_adhoc_to_external_station(author, content):
    """Στέλνει αυτόματα το μήνυμα του Ad-hoc στον εξωτερικό σταθμό μέσω Webhook POST."""
    with get_db_connection() as conn:
        cursor = conn.cursor()
...
# =====================================================================
# 4. ΣΤΥΛ, ΕΜΦΑΝΙΣΗ ΚΑΙ UI (MARINE THEME & ENHANCED BUTTONS)
# =====================================================================
BASE_STYLE = """
<style>
    body { 
    ...
    .channels-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(180px, 1fr)); gap: 10px; margin-top: 10px; }
    .channel-btn { display: block; background: #0284c7; color: white !important; padding: 12px; border-radius: 6px; text-align: center; font-weight: bold; text-decoration: none; box-shadow: 0 2px 5px rgba(0,0,0,0.2); }
    .channel-btn:hover { background: #0369a1; text-decoration: none; }
    .vessel-btn { display: block; background: #0d9488; color: white !important; padding: 12px; border-radius: 6px; text-align: center; font-weight: bold; text-decoration: none; box-shadow: 0 2px 5px rgba(0,0,0,0.2); }
    .vessel-btn:hover { background: #0f766e; text-decoration: none; }
...
<div style="max-width:1100px; margin:0 auto 10px; display:flex; justify-content:space-between; color:#e2e8f0; font-weight:bold;">
    <span>⚓ Σταθμοί Λάρνακας version 7.3 (Enhanced Button Grid UI)</span>
    {% if session.get('user_phone') %}<a href="/user_settings" style="color:white;">⚙️ Ρυθμίσεις Χρήστη</a>{% endif %}
</div>
"""

# =====================================================================
# 5. ΒΑΣΙΚΕΣ ΔΙΑΔΡΟΜΕΣ ΚΑΙ ΕΠΙΧΕΙΡΗΣΙΑΚΗ ΛΟΓΙΚΗ (ΜΕΡΟΣ Α' - DASHBOARD & GUIDE)
# =====================================================================

@app.route('/')
def index():
    """Κεντρική σελίδα πλοήγησης (Dashboard) με βελτιωμένη απεικόνιση καναλιών και σκαφών ως πλήκτρα."""
    ...
    <div class="container">
        ...
        <div class="channels-grid">
            {% for ch in standard_channels %}
                <a href="/logbook/{{ ch }}" class="channel-btn">
                    {% if ch == 'Γενικό Κανάλι' %}📢 {{ ch }}{% elif ch == 'Ad-hoc' %}🌐 {{ ch }} (Gateway){% else %}💬 {{ ch }}{% endif %}
                </a>
            {% endfor %}
        </div>
        <hr>
        ...
        <div class="channels-grid">
            {% for v in vessel_channels %}
                <a href="/vessel_log/{{ v }}" class="vessel-btn">🛥️ {{ v }}</a>
            {% endfor %}
        </div>
    </div>
...
@app.route('/guide')
def guide():
    """Σελίδα Γενικών Οδηγιών Χρήσης της Εφαρμογής με αναλυτικές οδηγίες για το προσωπικό."""
    if 'user_phone' not in session: return redirect(url_for('login'))
    return render_template_string(BASE_STYLE + """
    <div class="container">
        <h2>📖 Γενικές Οδηγίες Χρήσης Συστήματος</h2>
        <a href="/" class="btn-link btn-home">⮜⮜ Επιστροφή στην Αρχική</a>
        <hr>
        <h3>1. Σύνδεση & Εγγραφή</h3>
        <p>Κατά την πρώτη σας επίσκεψη στο σύστημα, κάνετε εγγραφή συμπληρώνοντας τα απαραίτητα στοιχεία. Ο πρώτος χρήστης κατοχυρώνεται αυτόματα ως <b>Super Administrator</b>, ενώ οι επόμενοι απαιτούν έγκριση.</p>
        
        <h3>2. Επιχειρησιακά Κανάλια & Logbooks</h3>
        <p>Τα κανάλια λειτουργούν ως ζωντανά ημερολόγια. Το κανάλι <b>Ad-hoc</b> λειτουργεί επιπλέον ως πύλη διασταθμικής επικοινωνίας (Inter-Station Gateway) με άλλους σταθμούς.</p>
        
        <h3>3. Real-time Beep Alerts</h3>
        <p>Κάθε 10 δευτερόλεπτα το σύστημα ελέγχει για νέα μηνύματα και αναπαράγει ηχητικό σήμα ειδοποίησης.</p>

        <h3>4. Διαχείριση Σκαφών</h3>
        <p>Καταγραφή ημερομηνίας, συμβάντων, βλαβών, ωρών κινητήρων και αναφορών επιδιόρθωσης για τα περιπολικά σκάφη.</p>

        <h3>5. Διασύνδεση Σταθμών (Webhooks)</h3>
        <p>Δήλωση του URL απομακρυσμένου σταθμού στο Πάνελ Διαχείρισης για κρυπτογραφημένη αμφίδρομη ροή στο Ad-hoc.</p>
        <hr>
        <a href="/" class="btn-link btn-home">⮜⮜ Επιστροφή στην Αρχική</a>
    </div>
    """)
...
def adhoc_webhook():
    """API Webhook Endpoint για ασφαλή λήψη μηνυμάτων από άλλους σταθμούς στο κανάλι Ad-hoc."""
    ...
    with get_db_connection() as conn:
        ...
        cursor.execute("INSERT INTO logbook_entries (channel_name, author_name, content, timestamp) VALUES ('Ad-hoc', ?, ?, ?)", (f"🌐 {data.get('author', 'Σταθμός')}", data.get('content', ''), data.get('timestamp', datetime.now().strftime('%Y-%m-%d %H:%M:%S'))))
        conn.commit()
    sync_database_to_supabase()
    return jsonify({"status": "success"}), 200
# =====================================================================
# 6. ΒΑΣΙΚΕΣ ΔΙΑΔΡΟΜΕΣ ΚΑΙ ΕΠΙΧΕΙΡΗΣΙΑΚΗ ΛΟΓΙΚΗ (ΜΕΡΟΣ Β' - AUTH & SETTINGS)
# =====================================================================

@app.route('/login', methods=['GET', 'POST'])
def login():
    """Διαχείριση σύνδεσης χρηστών στο σύστημα με επαλήθευση αριθμού τηλεφώνου και κωδικού πρόσβασης."""
    error_msg = ""
...
# =====================================================================
# 7. ΒΑΣΙΚΕΣ ΔΙΑΔΡΟΜΕΣ ΚΑΙ ΕΠΙΧΕΙΡΗΣΙΑΚΗ ΛΟΓΙΚΗ (ΜΕΡΟΣ Γ' - LOGBOOKS, VESSELS & ADMIN)
# =====================================================================
...
def logbook(channel_name):
    ...
    with get_db_connection() as conn:
        ...
        if request.method == 'POST':
            ...
            cursor.execute("INSERT INTO logbook_entries (channel_name, author_name, content, filename, mega_link, timestamp) VALUES (?, ?, ?, ?, ?, ?)", (channel_name, user['name'], content, fname, clink, datetime.now().strftime('%Y-%m-%d %H:%M:%S')))
            conn.commit(); sync_database_to_supabase()

            # Αν το μήνυμα στάλθηκε στο Ad-hoc, γίνεται προώθηση στον εξωτερικό σταθμό
...
def delete_logbook_message():
    """Διαγραφή μηνύματος καναλιού και του συνημμένου του αρχείου από το Cloud."""
    with get_db_connection() as conn:
        cursor = conn.cursor()
...
def admin():
    """Κεντρικό Πάνελ Διαχείρισης (Εγκρίσεις, webhooks, μετονομασία, ορφανά αρχεία)."""
    if 'user_phone' not in session: return redirect(url_for('login'))
    with get_db_connection() as conn:
        cursor = conn.cursor()
    ...
    <div class="container"><h2>🛠️ Πάνελ Διαχείρισης (v7.3)</h2><a href="/" class="btn-link btn-home">⮜⮜ Επιστροφή στην Αρχική</a>
        ...
        <form method="POST" style="background:#e0f2fe; padding:15px; border:1px solid #bae6fd; border-radius:5px; margin-bottom:15px;">
