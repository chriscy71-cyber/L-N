# =====================================================================
# 1. ΑΡΧΙΚΟΠΟΙΗΣΗ, ΒΑΣΗ ΔΕΔΟΜΕΝΩΝ & ΧΑΡΤΗΣ ΜΕΣΟΓΕΙΟΥ (v7.6 - Channel Rename)
# =====================================================================
import os
import sqlite3
import smtplib
...
# =====================================================================
# 2. ΔΙΑΧΕΙΡΙΣΗ ΑΡΧΕΙΩΝ, ΣΥΓΧΡΟΝΙΣΜΟΣ, WEBHOOKS & STYLE (v7.6)
# =====================================================================
...
<div style="max-width:1100px; margin:0 auto 10px; display:flex; justify-content:space-between; color:#e2e8f0; font-weight:bold;">
    <span>⚓ Σταθμοί Λάρνακας version 7.6 (Channel Management & Core)</span>
    ...
    if request.method == 'POST':
        ...
        if action == 'approve':
            ...
            if ch_id and new_name:
                with get_db_connection() as conn:
                    cursor = conn.cursor()
                    old_ch = cursor.execute("SELECT name FROM channels WHERE id = ?", (ch_id,)).fetchone()
                    if old_ch:
                        old_name = old_ch['name']
    ...
    <div class="container" style="max-width:1200px;">
        ...
        <div style="display:flex; gap:20px; flex-wrap:wrap;">
            ...
            <div style="flex:2; min-width:400px; background:#f8fafc; padding:15px; border-radius:8px; border:1px solid #cbd5e1;">
                ...
                <h3>💬 Διαχείριση Καναλιών & Μετονομασία</h3>
                <form method="POST" style="display:flex; gap:10px; margin-bottom:10px;">
                    <input type="hidden" name="action" value="add_channel">
                    <input type="text" name="channel_name" placeholder="Νέο όνομα καναλιού..." required>
                    <button type="submit" class="btn-link btn-success">➕ Προσθήκη</button>
                </form>
                <div style="max-height:200px; overflow-y:auto;">
                    <table>
                        <tr>
                            <th>Όνομα Καναλιού</th>
                            <th>Ενέργεια Μετονομασίας</th>
                        </tr>
                        {% for ch in channels %}
                        <tr>
                            <form method="POST">
                                <td>
                                    ...
                                    <input type="text" name="new_name" value="{{ ch['name'] }}" required style="margin:0;">
                                </td>
                                <td>
                                    <button type="submit" class="btn-link" style="padding:4px 8px; font-size:11px;">✏️ Αλλαγή</button>
                                </td>
                            </form>
                        </tr>
                        {% endfor %}
...
def guide():
    ...
    <div class="container" style="max-width:900px;">
        <h2>📖 Επιχειρησιακός Οδηγός Χρήσης Συστήματος v7.6</h2>
        <div class="nav-buttons">
            <a href="/" class="btn-link btn-home">🏠 Αρχική Σελίδα</a>
            <a href="/logout" class="btn-link btn-danger">🚪 Αποσύνδεση</a>
        </div>
        ...
        <p>Το σύστημα <b>Σταθμοί Λάρνακας v7.6</b> παρέχει πλήρη επιχειρησιακή υποστήριξη για την Λιμενική Αστυνομία, διασφαλίζοντας την αδιάλειπτη καταγραφή συμβάντων, διαχείριση σκαφών, ψηφιακή βιβλιοθήκη εγγράφων και διαχείριση βαρδιών (Roster).</p>
