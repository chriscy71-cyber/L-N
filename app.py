# =====================================================================
# 1. ΑΡΧΙΚΟΠΟΙΗΣΗ, ΒΑΣΗ ΔΕΔΟΜΕΝΩΝ & ΧΑΡΤΗΣ ΜΕΣΟΓΕΙΟΥ (v7.3)
# =====================================================================
import os
import sqlite3
import smtplib
...
def init_db():
    with get_db_connection() as conn:
        ...
        # Πίνακας για τον Επιχειρησιακό Χάρτη (Σημαίες / Markers)
        ...
        # Μετονομασία «Γενικού Καναλιού» σε «Συμβάντα»
        initial_channels = [
...
"""
# =====================================================================
# 3. DASHBOARD, ΚΑΝΑΛΙΑ, ΣΚΑΦΗ ΩΣ ΠΛΗΚΤΡΑ ΚΑΙ ΕΠΙΧΕΙΡΗΣΙΑΚΟΣ ΧΑΡΤΗΣ (v7.3)
# =====================================================================
...
@app.route('/map')
def operational_map():
    """Διαδραστικός Χάρτης Ανατολικής Μεσογείου με δυνατότητα τοποθέτησης και διαγραφής σημαιών."""
    if 'user_phone' not in session: return redirect(url_for('login'))
    with get_db_connection() as conn:
        cursor = conn.cursor()
    ...
    <!-- Modal Φόρμας Καταχώρησης Σημαίας -->
    ...
    <script>
        // Κεντράρισμα στην Ανατολική Μεσόγειο (Κύπρος / Λάρνακα)
...
def delete_marker():
    ...
    return redirect(url_for('operational_map'))
# =====================================================================
# 4. ΟΔΗΓΙΕΣ ΧΡΗΣΗΣ, LOGBOOKS ΜΕ ΚΑΤΗΓΟΡΙΕΣ ΚΑΙ WEBHOOKS (v7.3)
# =====================================================================
...
# =====================================================================
# 5. AUTHENTICATION, VESSEL LOGS, LIBRARY, ROSTER & ADMIN PANEL (v7.3)
# =====================================================================
...
# =====================================================================
# 2. ΔΙΑΔΡΟΜΕΣ & ΛΕΙΤΟΥΡΓΙΕΣ ΧΑΡΤΗ & ADMIN (v7.3)
# =====================================================================
MAP_TEMPLATE = '''
<!DOCTYPE html>
<html lang="el">
<head>
    <meta charset="UTF-8">
    <title>Επιχειρησιακός Χάρτης - Ανατολική Μεσόγειος</title>
    <link rel="stylesheet" href="https://unpkg.com/leaflet@1.9.4/dist/leaflet.css" />
    <style>
        body { font-family: Arial, sans-serif; margin: 0; padding: 0; background: #f4f6f9; }
        .header { background: #1b2a47; color: white; padding: 15px 20px; display: flex; justify-content: space-between; align-items: center; }
        #map { height: 85vh; width: 100%; }
        .leaflet-popup-content-wrapper { font-size: 14px; }
        .btn-back { background: #4e73df; color: white; padding: 8px 15px; text-decoration: none; border-radius: 4px; }
    </style>
</head>
<body>
    <div class="header">
        <h2>🗺️ Επιχειρησιακός Χάρτης Συμβάντων (Ανατ. Μεσόγειος)</h2>
        <div>
            <span>Χρήστης: <strong>{{ current_user }}</strong></span> &nbsp;|&nbsp;
            <a href="/" class="btn-back">Επιστροφή στο Dashboard</a>
        </div>
    </div>
    <div id="map"></div>
    ...
    <script src="https://unpkg.com/leaflet@1.9.4/dist/leaflet.js"></script>
    <script>
        // Κέντρο Ανατολικής Μεσογείου (Λάρνακα / Κύπρος)
        var map = L.map('map').setView([34.9175, 33.6360], 7);
        ...
        var markersData = {{ markers | tojson }};

        // Φόρτωση υπαρχόντων σημείων
        {% for m in markers %}
            var marker = L.marker([{{ m.lat }}, {{ m.lng }}]).addTo(map);
            var popupContent = "<b>Κατηγορία:</b> {{ m.incident_type }}<br>" +
                               "<b>Περιγραφή:</b> {{ m.description }}<br>" +
                               "<b>Καταχώρηση:</b> {{ m.author }} ({{ m.timestamp }})";
            
            {% if role == 'Admin' %}
                popupContent += "<br><form action='{{ url_for('delete_marker', marker_id=m.id) }}' method='POST' style='margin-top:5px;'><button type='submit' style='background:red; color:white; border:none; padding:3px 8px; cursor:pointer; border-radius:3px;'>🗑️ Διαγραφή Σημαίας</button></form>";
            {% endif %}
            
            marker.bindPopup(popupContent);
        {% endfor %}

        // Προσθήκη νέου σημείου με κλικ στον χάρτη
        map.on('click', function(e) {
            var lat = e.latlng.lat;
            var lng = e.latlng.lng;

            var formHtml = "<form action='{{ url_for('add_marker') }}' method='POST'>" +
                           "<input type='hidden' name='lat' value='" + lat + "'>" +
                           "<input type='hidden' name='lng' value='" + lng + "'>" +
                           "<b>Επιλογή Συμβάντος:</b><br>" +
                           "<select name='incident_type' style='width:100%; margin:5px 0; padding:5px;'>" +
                           "<option>🚨 Έρευνα & Διάσωση (SAR)</option>" +
                           "<option>⛵ Παράνομη Μετανάστευση</option>" +
                           "<option>🛢️ Θαλάσσια Ρύπανση</option>" +
                           "<option>🔒 Παραβίαση ISPS</option>" +
                           "<option>🔧 Τεχνικό / Άλλο Περιστατικό</option>" +
                           "</select><br>" +
                           "<b>Περιγραφή:</b><br>" +
                           "<textarea name='description' required style='width:100%; height:50px; margin:5px 0;'></textarea><br>" +
                           "<button type='submit' style='background:#1b2a47; color:white; border:none; padding:5px 10px; cursor:pointer; width:100%; border-radius:3px;'>Αποθήκευση Σημείου</button>" +
                           "</form>";
...
def operational_map():
    ...
    with get_db_connection() as conn:
        markers = conn.execute('SELECT * FROM map_markers ORDER BY timestamp DESC').fetchall()
        
    return render_template_string(MAP_TEMPLATE, markers=markers, current_user=session.get('name'), role=session.get('role'))
...
def add_marker():
    ...
    author = session.get('name', 'Άγνωστος')
    timestamp = datetime.now().strftime('%d/%m/%Y %H:%M')
    
    with get_db_connection() as conn:
        conn.execute('INSERT INTO map_markers (lat, lng, incident_type, description, author, timestamp) VALUES (?, ?, ?, ?, ?, ?)',
                     (lat, lng, incident_type, description, author, timestamp))
        conn.commit()
