...
def download_database_from_supabase():
    """Κατεβάζει τη βάση δεδομένων από το Supabase κατά την εκκίνηση του Render."""
    if not supabase:
        logging.warning("[SUPABASE] Λείπουν τα κλειδιά. Η βάση δεν κατέβηκε.")
        return False
    try:
        ...
        if any(f.get('name') == DB_NAME for f in files):
            ...
            with open(DB_NAME, 'wb') as f:
                f.write(res)
            logging.info("[SUPABASE] Η βάση δεδομένων ανακτήθηκε επιτυχώς.")
            return True
...
def sync_database_to_supabase():
    """Ανεβάζει το αρχείο database.db στο Supabase μετά από αλλαγές (αντικαθιστά το Mega)."""
    if not supabase or not os.path.exists(DB_NAME):
        return
    try:
        with open(DB_NAME, 'rb') as f:
            supabase.storage.from_(BUCKET_NAME).upload(file=f, path=DB_NAME, file_options={"upsert": "true"})
        logging.info("[SUPABASE] Η βάση δεδομένων συγχρονίστηκε στο Cloud.")
    except Exception as e:
        logging.error(f"[SUPABASE SYNC ERROR] {e}")

def get_supabase_storage_usage():
    """Υπολογίζει τον συνολικό χώρο (σε bytes) του Supabase Bucket (Όριο Free Tier: 1GB)."""
    if not supabase:
        return 0
    try:
        files = supabase.storage.from_(BUCKET_NAME).list()
        total_size = 0
        for f in files:
            if isinstance(f, dict) and 'metadata' in f and f['metadata'] is not None:
                size = f['metadata'].get('size', 0)
                total_size += size
        return total_size
    except Exception as e:
        logging.error(f"[STORAGE SIZE ERROR] {e}")
        return 0

# Πάγια εντολή: Κατέβασμα βάσης πριν ανοίξει η εφαρμογή
...
def init_db():
    with get_db_connection() as conn:
        ...
        CREATE TABLE IF NOT EXISTS logbook_entries (
            ...
            filename TEXT,
            mega_link TEXT,  -- Διατηρούμε το όνομα στήλης για λόγους συμβατότητας
        ...
        CREATE TABLE IF NOT EXISTS library (
            ...
            mega_link TEXT,  -- Διατηρούμε το όνομα στήλης
...
init_db()
# Αν δημιουργήθηκε τώρα η βάση (π.χ. άδειο bucket), κάνουμε το πρώτο sync.
...
def upload_file_to_supabase(file_path, filename):
    if not supabase:
        return None
    try:
        with open(file_path, 'rb') as f:
            supabase.storage.from_(BUCKET_NAME).upload(file=f, path=filename, file_options={"upsert": "true"})
        link = supabase.storage.from_(BUCKET_NAME).get_public_url(filename)
        return link
    except Exception as e:
        logging.error(f"[SUPABASE FILE UPLOAD ERROR] {e}")
        return None

def delete_file_from_supabase(filename):
    """Διαγράφει οριστικά ένα αρχείο από το Bucket του Supabase για αποδέσμευση χώρου."""
    if not supabase or not filename:
        return
    try:
        supabase.storage.from_(BUCKET_NAME).remove([filename])
        logging.info(f"[SUPABASE] Το αρχείο {filename} διαγράφηκε οριστικά από το Cloud.")
    except Exception as e:
...
<div class="app-brand-bar">
    <span>⚓ Σταθμοί Λάρνακας version 5.1 (Supabase Sync Active)</span>
    ...
    with get_db_connection() as conn:
        ...
        if request.method == 'POST' and role == 'Admin':
            ...
            if file and file.filename != '':
                ...
                cloud_link = upload_file_to_supabase(local_path, unique_filename)
                timestamp = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
                cursor.execute("INSERT INTO library (title, category, filename, mega_link, uploader, timestamp) VALUES (?, ?, ?, ?, ?, ?)",
                               (title, category, unique_filename, cloud_link, author_name, timestamp))
                conn.commit()
            sync_database_to_supabase()
    ...
    <div class="container">
        ...
        <table>
            ...
            <tr>
                ...
                <td>
                    {% if mlink %}
                        <a href="{{ mlink }}" target="_blank" class="btn-link" style="padding:4px 8px; font-size:11px;">📥 Cloud (Supabase)</a>
                    {% else %}
...
def delete_library_item():
    ...
    with get_db_connection() as conn:
        ...
        # 1. Διαγραφή του αρχείου από το Cloud
        cursor.execute("SELECT filename FROM library WHERE id = ?", (item_id,))
        row = cursor.fetchone()
        if row and row['filename']:
            delete_file_from_supabase(row['filename'])
...
def logbook(channel_name):
    ...
    with get_db_connection() as conn:
        ...
        if request.method == 'POST':
            ...
            if file and file.filename != '':
                ...
                file.save(local_path)
                cloud_link = upload_file_to_supabase(local_path, filename)
                
            timestamp = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
            
            cursor.execute("INSERT INTO logbook_entries (channel_name, author_name, content, filename, mega_link, timestamp) VALUES (?, ?, ?, ?, ?, ?)",
                           (channel_name, author_name, content, filename, cloud_link, timestamp))
            conn.commit()
            sync_database_to_supabase()
    ...
    <div class="container">
        ...
        <form method="POST" enctype="multipart/form-data" style="background:#f1f5f9; padding:15px; border:1px solid #cbd5e1; border-radius:5px;">
            ...
            Επισύναψη Αρχείου (Cloud Supabase): <input type="file" name="file"><br><br>
            ...
            <div class="viber-chat-container">
                {% for entry_id, author, content, filename, mlink, time in entries %}
                    <div class="chat-bubble">
                        ...
                        {% if mlink %}
                            <div style="margin-top:6px;"><a href="{{ mlink }}" target="_blank" class="btn-link" style="padding:2px 6px; font-size:11px;">📥 Λήψη (Supabase)</a></div>
                        {% elif filename %}
            ...
            <ul>
                {% for entry_id, author, content, filename, mlink, time in entries %}
                    <li style="margin-bottom: 10px; background:#fff; padding:10px; border-radius:4px; border:1px solid #e2e8f0;">
                        ...
                        {% if mlink %}
                            <br>📎 <i>Αρχείο (Cloud):</i> <a href="{{ mlink }}" target="_blank" class="btn-link" style="padding:2px 6px; font-size:11px; margin-top:5px;">📥 Λήψη</a>
                        {% elif filename %}
...
def delete_logbook_message():
    ...
    with get_db_connection() as conn:
        ...
        # 1. Βρίσκουμε αν υπάρχει συνημμένο αρχείο και το διαγράφουμε από το Cloud
        cursor.execute("SELECT filename FROM logbook_entries WHERE id = ?", (entry_id,))
        row = cursor.fetchone()
        if row and row['filename']:
            delete_file_from_supabase(row['filename'])
...
def admin():
    ...
    # [Προσθήκη ελέγχου χωρητικότητας Supabase]
    used_bytes = get_supabase_storage_usage()
    limit_bytes = 1024 * 1024 * 1024  # 1 GB δωρεάν όριο
    ...
    <div class="container">
        ...
        <!-- ΜΠΑΡΑ ΕΙΔΟΠΟΙΗΣΗΣ ΧΩΡΗΤΙΚΟΤΗΤΑΣ -->
        {% if usage_percent >= 90 %}
        <div style="background:#fef2f2; border:1px solid #ef4444; padding:15px; border-radius:5px; margin-bottom:20px;">
            <h3 style="color:#b91c1c; margin-top:0;">⚠️ ΚΡΙΣΙΜΗ ΠΡΟΕΙΔΟΠΟΙΗΣΗ ΧΩΡΗΤΙΚΟΤΗΤΑΣ</h3>
            <p style="color:#991b1b; font-weight:bold; margin-bottom:0;">
                Ο αποθηκευτικός χώρος του Cloud βρίσκεται στο {{ "%.1f"|format(usage_percent) }}% ({{ "%.1f"|format(used_mb) }} MB από 1024 MB).<br>
                Απαιτείται άμεση διαγραφή παλαιών αρχείων/αναφορών για την αποφυγή παύσης λειτουργίας!
            </p>
        </div>
        {% else %}
        <div style="background:#f0fdf4; border:1px solid #22c55e; padding:10px; border-radius:5px; margin-bottom:20px; font-size:12px; color:#15803d; font-weight:bold;">
    ...
    """, users=users, msg=msg, roster_active=is_roster_active(), super_admin_id=super_admin_id, is_super_admin=is_super_admin, used_mb=used_mb, usage_percent=usage_percent)
