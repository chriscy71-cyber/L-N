...
def init_db():
    with get_db_connection() as conn:
        ...
        cursor.execute("INSERT OR IGNORE INTO settings (key, value) VALUES ('admin_password', ?)", (DEFAULT_ADMIN_PASSWORD,))
...
def register():
    if request.method == 'POST':
        ...
        try:
            with get_db_connection() as conn:
                cursor = conn.cursor()
                # Έλεγχος αν είναι ο πρώτος χρήστης στο σύστημα
                cursor.execute("SELECT COUNT(*) FROM users")
                user_count = cursor.fetchone()[0]
                
                if user_count == 0:
                    # Ο πρώτος γίνεται αυτόματα Admin και Approved
                    cursor.execute("INSERT INTO users (phone, name, station, status, role) VALUES (?, ?, ?, 'Approved', 'Admin')",
                                   (phone, name, station))
                else:
                    # Οι επόμενοι γίνονται Pending User
                    cursor.execute("INSERT INTO users (phone, name, station, status, role) VALUES (?, ?, ?, 'Pending', 'User')",
                                   (phone, name, station))
                conn.commit()
    ...
    with get_db_connection() as conn:
        ...
        if request.method == 'POST':
            if channel_name == 'E-Καθήκοντα':
                ...
                if admin_pass != get_current_admin_password():
                    return render_template_string(BASE_STYLE + "<div class='container'><h3 style='color:red;'>Σφάλμα: Απαιτείται έγκυρος Κωδικός Διαχειριστή για καταχώρηση στα E-Καθήκοντα.</h3><a href='/logbook/E-Καθήκοντα'>Επιστροφή</a></div>")
...
def admin():
    ...
    with get_db_connection() as conn:
        ...
        if request.method == 'POST':
            ...
            current_pass = get_current_admin_password()
    ...
    <div class="container">
        ...
        <form method="POST" style="background:#f1f5f9; padding:15px; border:1px solid #cbd5e1; border-radius:5px;">
            ...
            <table>
                <tr><th>Όνομα</th><th>Τηλέφωνο</th><th>Σταθμός</th><th>Κατάσταση</th><th>Ενέργεια</th></tr>
                {% for u in users %}
                <tr>
    ...
    """, users=users, channels_raw=channels_raw, channel_rosters=channel_rosters, msg=msg, roster_active=is_roster_active())
