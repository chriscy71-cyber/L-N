...
# Εγγυημένη ανάκτηση βάσης πριν από κάθε αίτημα (Λύνει οριστικά το πρόβλημα του Render Sleep)
@app.before_request
...
<div style="max-width:1100px; margin:0 auto 10px; display:flex; justify-content:space-between; color:#e2e8f0; font-weight:bold;">
    <span>⚓ Σταθμοί Λάρνακας version 7.2 (Admin Error Shield)</span>
...
def index():
    ...
    <div class="container">
        ...
        <ul>{% for ch in standard_channels %}<li><a href="/logbook/{{ ch }}">{% if ch == 'Γενικό Κανάλι' %}<b>📢 {{ ch }}</b>{% elif ch == 'Ad-hoc' %}<b>🌐 {{ ch }} (Inter-Station Gateway)</b>{% else %}💬 {{ ch }}{% endif %}</a></li>{% endfor %}</ul>
        <hr>
...
def register():
    ...
    if request.method == 'POST':
        ...
        except sqlite3.IntegrityError: msg = 'Το τηλέφωνο υπάρχει ήδη.'
...
def admin():
    """Κεντρικό Πάνελ Διαχείρισης (Προστατευμένο με Try-Except Shield)."""
    if 'user_phone' not in session: return redirect(url_for('login'))
    try:
        with get_db_connection() as conn:
            cursor = conn.cursor()
        ...
        return render_template_string(BASE_STYLE + "<div class='container'><h2>⚠️ Σφάλμα Διαχειριστικού Πάνελ</h2><p>Παρουσιάστηκε εσωτερικό σφάλμα κατά τη φόρτωση του πάνελ: {{ err }}</p><a href='/' class='btn-link btn-home'>⮜⮜ Επιστροφή στην Αρχική</a></div>", err=str(e))
        
    return render_template_string(BASE_STYLE + """
    <div class="container"><h2>🛠️ Πάνελ Διαχείρισης (v7.2)</h2><a href="/" class="btn-link btn-home">⮜⮜ Επιστροφή στην Αρχική</a>
        <p style="color:red; font-weight:bold;">{{ msg }}</p>
