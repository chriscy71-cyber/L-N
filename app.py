# =====================================================================
# 1. ΑΡΧΙΚΟΠΟΙΗΣΗ, ΒΑΣΗ ΔΕΔΟΜΕΝΩΝ & ΧΑΡΤΗΣ ΜΕΣΟΓΕΙΟΥ (v7.5 - 5/0 VERIFIED)
# =====================================================================
import os
import sqlite3
import smtplib
...
# =====================================================================
# 2. ΔΙΑΧΕΙΡΙΣΗ ΑΡΧΕΙΩΝ, ΣΥΓΧΡΟΝΙΣΜΟΣ, WEBHOOKS & STYLE (v7.5)
# =====================================================================
...
<div style="max-width:1100px; margin:0 auto 10px; display:flex; justify-content:space-between; color:#e2e8f0; font-weight:bold;">
    <span>⚓ Σταθμοί Λάρνακας version 7.5 (5/0 Verified Core)</span>
...
def register():
    if request.method == 'POST':
        ...
        if not phone or not name or not station:
            flash("Συμπληρώστε τα υποχρεωτικά πεδία (Τηλέφωνο, Ονοματεπώνυμο, Σταθμός).")
        ...
        with get_db_connection() as conn:
            ...
            if existing:
                flash("Ο αριθμός τηλεφώνου υπάρχει ήδη. Παρακαλώ συνδεθείτε.")
            ...
            pass_hash = generate_password_hash(password) if password else ''

            cursor.execute("""
                INSERT INTO users (phone, name, station, role, status, admin_level, password, email)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            """, (phone, name, station, role, status, admin_level, pass_hash, email))
            conn.commit()
        sync_database_to_supabase()
        flash("Η εγγραφή σας καταχωρήθηκε επιτυχώς στη βάση δεδομένων. Αναμένεται έγκριση από τη Διαχείριση.")
        return redirect(url_for('login'))
    ...
    <div class="container" style="max-width:400px; margin-top:60px;">
        ...
        <form method="POST">
            ...
            <input type="text" name="name" placeholder="π.χ. Ιωάννης Παπαδόπουλος" required>
            ...
            <input type="text" name="station" placeholder="π.χ. Λιμάνι Λάρνακας" required>
            <label><b>Email:</b></label>
            <input type="email" name="email" placeholder="name@police.gov.cy">
            <label><b>Κωδικός Πρόσβασης:</b></label>
            <input type="password" name="password" placeholder="••••••••" required>
            <button type="submit" class="btn-link btn-success" style="width:100%; margin-top:15px;">💾 Άμεση Εγγραφή στη Βάση</button>
        </form>
        <p style="text-align:center; margin-top:15px;"><a href="/login" style="color:#0056b3; font-weight:bold;">← Επιστροφή στην Είσοδο</a></p>
    </div>
    """)
...
def login():
    if request.method == 'POST':
        ...
        if not user:
            flash("Ο χρήστης δεν βρέθηκε. Παρακαλώ κάντε εγγραφή πρώτα.")
            return redirect(url_for('register'))

        if user['role'] == 'Admin':
            if password == DEFAULT_ADMIN_PASSWORD or (user['password'] and check_password_hash(user['password'], password)) or (user['password'] == password):
            ...
            if user['password'] and not check_password_hash(user['password'], password) and user['password'] != password:
                flash("Λανθασμένος κωδικός πρόσβασης.")
            else:
                session['user_phone'] = user['phone']
                session['user_name'] = user['name']
                session['role'] = user['role']
    ...
    <div class="container" style="max-width:400px; margin-top:80px;">
        ...
        <form method="POST">
            ...
            <button type="submit" class="btn-link btn-success" style="width:100%; margin-top:15px;">🔐 Είσοδος</button>
        </form>
        <p style="text-align:center; margin-top:15px;">Δεν έχετε λογαριασμό; <a href="/register" style="color:#0056b3; font-weight:bold;">Εγγραφή εδώ</a></p>
    </div>
    """)
...
def user_settings():
    ...
    if request.method == 'POST':
        ...
        with get_db_connection() as conn:
            cursor = conn.cursor()
            if new_pass:
                pass_h = generate_password_hash(new_pass)
                cursor.execute("UPDATE users SET name = ?, station = ?, email = ?, password = ? WHERE phone = ?", (name, station, email, pass_h, session['user_phone']))
            else:
    ...
    <div class="container" style="max-width:1200px;">
        ...
        <div style="display:flex; gap:20px; flex-wrap:wrap;">
            ...
            <div style="flex:2; min-width:400px; background:#f8fafc; padding:15px; border-radius:8px; border:1px solid #cbd5e1;">
                <h3>👥 Διαχείριση & Έλεγχος Προσωπικού (Users)</h3>
...
def guide():
    ...
    <div class="container" style="max-width:900px;">
        <h2>📖 Επιχειρησιακός Οδηγός Χρήσης Συστήματος v7.5</h2>
        <div class="nav-buttons">
            <a href="/" class="btn-link btn-home">🏠 Αρχική Σελίδα</a>
            <a href="/logout" class="btn-link btn-danger">🚪 Αποσύνδεση</a>
        </div>
        ...
        <p>Το σύστημα <b>Σταθμοί Λάρνακας v7.5</b> παρέχει πλήρη επιχειρησιακή υποστήριξη για την Λιμενική Αστυνομία, διασφαλίζοντας την αδιάλειπτη καταγραφή συμβάντων, διαχείριση σκαφών, ψηφιακή βιβλιοθήκη εγγράφων και διαχείριση βαρδιών (Roster).</p>
